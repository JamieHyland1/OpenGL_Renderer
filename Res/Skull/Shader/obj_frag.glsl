...
//     vec4 col = texture(test, TexCoords);
//     // col += vec4(aPos2,1.0f);
...
{
   // TexCoord *= -1;
    FragColor = texture(texture2, TexCoord);//mix(texture(ourTexture, TexCoord + sin(time)), texture(texture2, vec2(TexCoord.x*5, TexCoord.y * 5)), 1);
}